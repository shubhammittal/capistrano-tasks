...
describe CapistranoTasks::Unicorn do
  ...
  it "should be able to reduce workers to zero, and then port would be closed obviously" do
    ...
    assert !http_open?(3000), "port should no longer be opened since there's no worker"
    run_task "test_c:increment"
    sleep 5
    system("ps aux | grep unicorn")
    assert http_open?(3000)
    safe_stop
  end

  it "should be able to stop if old master has zero workers" do
    run_task("test_c:start")
    sleep 10
    assert http_open?(3000)
    run_task "test_c:decrement"
  ...
  it "should be able to restart even if old master has zero workers" do
    setup_configuration(:bootup_timeout => 10)
    run_task("test_c:start")
    sleep 5
    assert http_open?(3000)
  ...
  def safe_stop
    run_task("test_c:graceful_stop")
    run_task("test_c:wait_till_dead")
    port_open?(3000).must_equal(false, "port should be back closed now")
  end
